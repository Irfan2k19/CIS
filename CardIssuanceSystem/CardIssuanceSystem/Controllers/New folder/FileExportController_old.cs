...
namespace CardIssuanceSystem.Controllers
{
    public class FileExportController : BaseController
    {
        ...
        private CardData GetCardDataNew(int dataCount, RequestForExportVM requestDetail, string action)
        {
            string address = string.IsNullOrEmpty(requestDetail.AssociatedAddress) ? requestDetail.AssociatedAddress : requestDetail.AssociatedAddress.Insert(requestDetail.AssociatedAddress.Length, "$").Replace(" ", "*");
            ...
            return new CardData()
            {
                ...
                RecordNumber = dataCount.ToString().PadLeft(6, '0'),
                RecordType = Constants.RecordTypes.CardData.LimitTo(2),
                ActionCode = action.LimitTo(99),
                CustomerId = CustomerNIC,
                EmbossingName = requestDetail.NameOnCard.LimitTo(30),
                Customer0rStaff = "1".LimitTo(1),
                PrimaryCNIC = CustomerNIC,
        ...
        private CustomerProfile GetProfileDataNew(int dataCount, RequestForExportVM requestDetail, string action)
        {
            string CustomerNIC = string.IsNullOrEmpty(requestDetail.NIC) ? string.Empty.LimitTo(20) : requestDetail.NIC.Replace("-", "").LimitTo(20);
            return new CustomerProfile()
            {
        ...
        private AccountData GetAccountDataNew(int dataCount, RequestForExportVM requestDetail, string branchCode, string action)
        {
            string CustomerNIC = string.IsNullOrEmpty(requestDetail.NIC) ? string.Empty.LimitTo(20) : requestDetail.NIC.Replace("-", "").LimitTo(20);
            return new AccountData()
            {
        ...
        private void AccountSectionExtensionNew(ref List<AccountData> accounts, string accountNo, string accountTitle, string accountType, string branchCode, string requestType, int dataCount, string pan)
        {
            var accDetail = new AccountData()
            {
                AccountNo = accountNo,
        ...
        private void AccountSectionExtensionForLinkingNew(ref List<AccountData> accounts, string accountNo, string accountTitle, string accountType, string branchCode, string requestType, int dataCount, string pan, string Currency)
        {
            var accDetail = new AccountData()
            {
                AccountNo = accountNo,
